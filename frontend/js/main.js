// --- FILE JAVASCRIPT MOI (DA GOP LOGIC) ---
// Bao gom:
// 1. Logic dieu huong (Scroll) tu "TeenTalker"
// 2. Logic goi API (STT, Text, TTS) tu "AI Speaking Coach"
...
const micIcon = iconTemplates.querySelector('#svg-mic-icon').cloneNode(true);
const stopIcon = iconTemplates.querySelector('#svg-stop-icon').cloneNode(true);
const playIcon = iconTemplates.querySelector('#svg-play-icon').cloneNode(true);
const pauseIcon = iconTemplates.querySelector('#svg-pause-icon').cloneNode(true);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 4. Gan su kien cho Nav & Scroll (Tu "TeenTalker")
    ...
    // 5. Kich hoat hieu ung cuon (Tu "TeenTalker")
    activateScrollEffects();
});

// --- LOGIC TAO CHU DE (Giu nguyen) ---
...
// --- LOGIC GHI AM (MediaRecorder) (Giu nguyen) ---
...
// --- LOGIC GOI API CHINH (Backend) (Giu nguyen) ---
...
async function sendAudioToAPI(audioBlob) {
    ...
    try {
        // 3. Gửi API (Backend /api/process-speech)
...
// --- HAM CHUYEN DOI PCM SANG WAV (BAT BUOC) (Giu nguyen) ---
